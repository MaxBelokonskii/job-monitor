...
def test_no_cors_middleware_is_installed() -> None:
    ...
    Поведение — главное, и оно ловит даже возврат middleware с пустым
    списком (`allow_origins=[]`): предполётный `OPTIONS` начинает
    отвечать 200 с `Access-Control-Allow-Methods`, проверено мутацией.
    Но пустой список безвреден лишь сам по себе — он готовое место, куда
    следующая правка допишет источник, а `["*"]` от него на один символ.
    Дешевле не пускать саму строку обратно, и сказать об этом прямо, чем
    рассчитывать, что читатель поймёт отказ предполёта.
    """
