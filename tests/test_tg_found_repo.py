...
def test_the_same_message_id_in_another_channel_is_a_different_post(conn) -> None:
    repo = TgFoundRepo(conn)
    assert repo.record("a", 1, [], None, None, NOW) is True
    assert repo.record("b", 1, [], None, None, NOW) is True


def test_recent_returns_newest_first(conn) -> None:
    repo = TgFoundRepo(conn)
    repo.record("a", 1, [], "старый", None, datetime(2026, 9, 1, 10, 0, 0))
    repo.record("a", 2, [], "новый", None, datetime(2026, 9, 2, 10, 0, 0))
    assert [row["preview"] for row in repo.recent(10)] == ["новый", "старый"]


def test_recorded_fields_read_back(conn) -> None:
    repo = TgFoundRepo(conn)
    repo.record("qajobs", 7, ["@hr_anna", "@lead"], "Ищем QA", "qa", NOW)
    row = repo.recent(1)[0]
    assert row["channel"] == "qajobs"
    assert row["message_id"] == 7
    # Множественное число не косметика: миграция 004 завела `usernames`
    # JSON-массивом, потому что контактов в одном посте бывает несколько.
    # Старая колонка `username` осталась и с тех пор не пишется.
