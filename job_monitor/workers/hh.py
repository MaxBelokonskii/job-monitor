    ...
    try:
        ...
        while not stop_event.is_set():
            ...
            try:
                ...
                # Лимит не мешает СОБИРАТЬ. Он существует потому, что
                # банят за отправку; в безопасном режиме отправки нет,
                # `applied_on` не растёт, и уснуть на десять минут значило
                # бы перестать наполнять очередь — то есть отменить ровно
