...
def build_search_url(profession: str, criteria: SearchCriteria) -> str:
    ...
    **`search_field=name` — поиск по названию должности, а не по всему
    тексту вакансии.** Без него по запросу «Frontend-разработчик»
    приходили Senior PHP Developer, Java-разработчик, DevOps и системный
    аналитик: hh.ru по умолчанию ищет и в описании, а упомянуть фронтенд
    там может кто угодно. Измерено на живом сайте с настоящими критериями:
    39 посторонних записей из 64 — шестьдесят один процент. С параметром —
    19 вакансий, все девятнадцать по делу.
    ...
    создаёт. Размен — полнота: вакансия с названием «Разработчик
    интерфейсов» не найдётся, пока такое название не добавлено в профессии;
    интерфейс поэтому и предлагает их списком, а не одной строкой.
    """
    params = [
...
def apply_to_vacancy(
    ...
    try:
        ...
        if resume_id and not _choose_resume(driver, resume_id):
            ...
            # выбрал, и отозвать это нельзя.
            #
            # Не откликаемся вовсе. Вакансия получит статус «пропущено»
            # (`_process_one`), а он входит в `statuses.REOPENABLE` — значит
            # на экране «Найдено» её видно среди всех и можно вернуть в
            # очередь кнопкой, а потом откликнуться руками. Причина при этом
            # видна только в журнале hh.ru (Настройки → Журнал работы), не в
            # карточке: отдельного статуса под «не нашёл резюме» в словаре
            # нет, и заводить его ради ненастроенного поля не стоит.
            log.warning(
