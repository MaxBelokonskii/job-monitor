...
Настройки, отклики и вакансии живут в SQLite, схема версионируется, секреты остались в окружении, `config.json` больше не источник правды (L7 закрыт). Метрики по-прежнему считаются парсингом логов, а воркеры остаются подпроцессами — это план 3.
